...
console.log('✅ Test document created at:', testFilePath);
console.log('📄 Content length:', testContent.length, 'characters');
console.log('🔍 Use this file to test the upload process with debugging enabled');

// Create a simple upload test script
...
async function testUpload() {
  ...
  // Create a test file
  const testFile = new File(['${testContent.replace(/'/g, "\\'")}'], 'test-document.txt', {
    type: 'text/plain'
  });
...
fs.writeFileSync(path.join(__dirname, 'test-upload.js'), uploadTestScript);
console.log('🧪 Upload test script created at:', path.join(__dirname, 'test-upload.js'));
console.log('\n📋 To test the upload process:');
console.log('1. Start your development server');
console.log('2. Open browser console on your app');
console.log('3. Copy and paste the content of test-upload.js');
console.log('4. Watch the detailed debug logs in your server console');
console.log('5. Check /api/debug/system-health for overall system status');